...
export function exportSingleScenarioXLSX(input: ExportSingleScenarioInput): void {
  ...
  const providerReportSections: ReportSection[] = [
    ...
    },
    {
      ...
      rows: [
        ...
        ['Other Incentive 3', formatCurrency(provider?.otherIncentive3)],
  ...
  const providerRawRows: Record<string, string | number>[] = provider
    ? [{
        ...
        otherIncentive1: safe(provider.otherIncentive1),
        otherIncentive2: safe(provider.otherIncentive2),
        otherIncentive3: safe(provider.otherIncentive3),
